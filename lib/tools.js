 ...
 */
export function resolveWriteGuard(collections, collectionId, protectedList) {
...
export function outlineResolvePathTool(makeClient) {
    return defineTool({
        ...
        description: '把用户描述的知识库路径（如"运维文档/个人笔记/随手记黄继晨"）解析为具体的 collectionId 与 parentDocumentId，并返回解析出的完整路径。用于定位"在某某目录下创建文档"的目标位置；解析成功后把返回的 id 传给 outline_create（创建前用户还会看到完整路径确认）。',
        parameters: {
            path: { type: 'string', required: true, description: '路径，用 / 分隔：第一段是集合名，后续段是逐级目录（文档）名，如 运维文档/个人笔记/随手记黄继晨' },
        },
        output: {
            schema: {
                type: 'object',
...
export function outlineCreateTool(makeClient, getProtected) {
    return defineTool({
        name: 'outline_create',
...
export function outlineUpdateDocumentTool(makeClient, getProtected) {
    return defineTool({
...
export function outlineDeleteTool(makeClient, getProtected, requestApproval) {
    return defineTool({
        name: 'outline_delete',
