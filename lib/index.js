...
export function apply(ctx, config = {}) {
    // ── Service guards ──────────────────────────────────────────────────
    // DSH may rename or defer these across releases; degrade safely rather
    // than throw at apply time. Each guard mirrors the contract documented
    // in the README.
    const tools = ctx.tools;
    if (tools === undefined || typeof tools.register !== 'function') {
        console.warn('[dsh-outline-auto] ctx.tools unavailable; tools not registered');
        return;
    }
    ...
    // installSettingsSection may be unavailable on newer DSH (settings API
    // reshaped): fall back to config-only mode so search/read tools still work.
    try {
        ...
        console.warn('[dsh-outline-auto] settings registration failed, using config-only mode: '
    ...
    ctx.tools.register(outlineDeleteTool(makeClient, getWritablePaths, async (reason, exec) => {
        ...
        if (approval === undefined) {
            console.warn('[dsh-outline-auto] approval service unavailable; delete requests denied by default');
            return false;
        }
        if (typeof approval.request !== 'function') {
            console.warn('[dsh-outline-auto] approval.request API shape changed; delete requests denied by default');
            return false;
        }
        try {
            ...
            console.warn('[dsh-outline-auto] approval.request threw; denying delete: '
                + (err && err.message ? err.message : err));
            return false;
        }
    }));
    // 写工具审批闸：create/update/delete 需用户确认；目录白名单或权限不满足时直接拒绝（连审批都不弹）。
    // Guard the event hook so a renamed event in a future DSH silently disables
    // the approval gate instead of throwing at every tool call.
    if (typeof ctx.on !== 'function') {
        console.warn('[dsh-outline-auto] ctx.on unavailable; write-tool approval gate disabled');
    }
    else {
        ...
        return { kind: 'ask', reason: `将删除 Outline 文档（第 1 次确认）：\n${where}` };
            ...
            // Event-signature skew (future DSH): fail closed — deny the write
            // rather than let it run without the whitelist + approval checks.
            return { kind: 'deny', reason: '写入审批闸内部错误（已拒绝写入以保护数据）。' };
        }
    });
    }
}
